...
namespace FloralHaven.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [HttpPost]
        [Route("Account/GetOrdersDetails")]
        public JsonResult GetOrdersDetails()
        {
            ...
            IQueryable<BILLDETAIL> oderDetails = _db.BILLDETAILs;
            ...
            if (!string.IsNullOrEmpty(order) && !string.IsNullOrWhiteSpace(order) && !string.IsNullOrEmpty(orderDir) && !string.IsNullOrWhiteSpace(orderDir))
            {
                switch (order)
                {
                    case "1":
                        if (orderDir == "asc")
                            oderDetails = oderDetails.OrderBy(p => p.productid);
                        else if (orderDir == "desc")
                            oderDetails = oderDetails.OrderByDescending(p => p.productid);
                        break;
                    case "2":
                        if (orderDir == "asc")
                            oderDetails = oderDetails.OrderBy(p => p.quantity);
                        else if (orderDir == "desc")
                            oderDetails = oderDetails.OrderByDescending(p => p.quantity);
                        break;
                    case "3":
                        if (orderDir == "asc")
                            oderDetails = oderDetails.OrderBy(p => p.price);
                        else if (orderDir == "desc")
                            oderDetails = oderDetails.OrderByDescending(p => p.price);
                        break;
                    default:
                        break;
                }
