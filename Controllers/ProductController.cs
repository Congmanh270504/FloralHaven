...
namespace FloralHaven.Controllers
{
    public class ProductController : Controller
    {
     ...
     .Select(p => new ProductViewModel
     {
         ...
         Images = _db.IMAGEs.Where(image => image.productid == p.id ).Select(image => image.path).ToList(),
         CategorySlug = (_db.CATEGORies.FirstOrDefault(category => category.id == p.categoryid)?.name) ?? "",
     ...
     .ToList();
            ...
            // Add the subItem list to the ViewBag
            var listSubItem = new List<ProductViewModel>(subItem);
