	...
	n.length &&
		((t = n.DataTable({
			...
			ajax: function (data, callback, settings) {
				// Generate the URL based on the page number and page length
				var url = "/Product/GetProducts?page=" + (data.start / data.length + 1) + "&pageSize=" + data.length;
				...
				$.ajax({
					...
					success: function (response) {
						// Pass the response data to the DataTable callback function
						callback(response);						
					},
			...
			columns: [ { data: "product_name" }, { data: "category" }, { data: "stock" }, { data: "sku" }, { data: "price" }, { data: "saleprice" }, { data: "" }],
			columnDefs: [
				{
					className: "control",
					searchable: !1,
				...
				},				
				{
					...
					render: function (t, e, s, a) {
						var n = s.product_name,
							...
							o = "",
							c = s.image;
						return '<div class="d-flex justify-content-start align-items-center product-name"><div class="avatar-wrapper"><div class="avatar avatar me-4 rounded-2 bg-label-secondary">' + (c ? '<img src="' + c + '" alt="Product-' + i + '" class="rounded-2">' : '<span class="avatar-initial rounded-2 bg-label-' + ["success", "danger", "warning", "info", "dark", "primary", "secondary"][Math.floor(6 * Math.random())] + '">' + (c = (((c = (n = "").match(/\b\w/g) || []).shift() || "") + (c.pop() || "")).toUpperCase()) + "</span>") + '</div></div><div class="d-flex flex-column"><h6 class="text-nowrap mb-0">' + n + '</h6><small class="text-truncate d-none d-sm-block">' + o + "</small></div></div>";
					},
				},
				{
					targets: 2,
				...
				},				
				{
					...
					orderable: !1,
					render: function (t, e, s, a) {
						return '<div class="d-inline-block text-nowrap"><a href="/Product/Edit/'+ s.id +'" class="btn btn-sm btn-icon btn-text-secondary rounded-pill waves-effect waves-light"><i class="ti ti-edit ti-md"></i></a><button class="btn btn-sm btn-icon btn-text-secondary rounded-pill waves-effect waves-light dropdown-toggle hide-arrow" data-bs-toggle="dropdown"><i class="ti ti-dots-vertical ti-md"></i></button><div class="dropdown-menu dropdown-menu-end m-0"><a href="/Product/'+ s.handle +'" class="dropdown-item">View</a><a href="javascript:0;" class="dropdown-item">Suspend</a></div></div>';
					},
				},
			],
			order: [1, "asc"],
